...
class GoogleNews:
    ...
    def build_response(self):
        self.req = urllib.request.Request(self.url.replace(
            "search?", "search?hl="+self.__lang+"&gl="+self.__lang+"&"), headers=self.headers)
        ...
        if stats and isinstance(stats, ResultSet):
            ...
            return
        result = self.content.find_all(
            "a", {"jsname": re.compile(r".*")})[3:-1]
        return result
        ...
        try:
            if self.__start != "" and self.__end != "":
                self.url = "https://www.google.com/search?q={}&lr=lang_{}&biw=1920&bih=976&source=lnt&&tbs=lr:lang_1{},cdr:1,cd_min:{},cd_max:{},sbd:1&tbm=nws&start={}".format(
                    self.__key, self.__lang, self.__lang, self.__start, self.__end, (10 * (page - 1)))
            elif self.__period != "":
                self.url = "https://www.google.com/search?q={}&lr=lang_{}&biw=1920&bih=976&source=lnt&&tbs=lr:lang_1{},qdr:{},,sbd:1&tbm=nws&start={}".format(
                    self.__key, self.__lang, self.__lang, self.__period, (10 * (page - 1)))
            else:
            ...
            raise AttributeError(
                "You need to run a search() before using get_page().")
        try:
            result = self.build_response()
            for item in result:
                try:
                    tmp_text = item.find(
                        "div", {"role": "heading"}).text.replace("\n", "")
                except Exception:
                    tmp_text = ''
                try:
                ...
                try:
                    tmp_date = item.find(
                        "div", {"role": "heading"}).next_sibling.findNext('div').text
                ...
                try:
                    tmp_desc = item.find(
                        "div", {"role": "heading"}).next_sibling.text
                except Exception:
                    tmp_desc = ''
                try:
                ...
                results.append({'title': tmp_text, 'media': tmp_media, 'date': tmp_date, 'datetime': define_date(
                    tmp_date), 'desc': tmp_desc, 'link': tmp_link, 'img': tmp_img})
            self.response.close()
    ...
    def get_page(self, page=1):
        ...
        try:
            ...
            for item in result:
                ...
                try:
                    tmp_desc = item.find(
                        "div", {"role": "heading"}).next_sibling.text.replace('\n', '')
                except Exception:
                    tmp_desc = ''
                try:
                    tmp_img = item.findAll("g-img")[0].find("img").get("src")
                ...
                self.__texts.append(tmp_text)
                self.__links.append(tmp_link)
                self.__results.append({'title': tmp_text, 'media': tmp_media, 'date': tmp_date, 'datetime': define_date(
                    tmp_date), 'desc': tmp_desc, 'link': tmp_link, 'img': tmp_img})
            self.response.close()
        except Exception as e_parser:
            print(e_parser)
    ...
    def get_news(self, key="", deamplify=False):
        ...
        self.url = 'https://news.google.com/search?q={}+after:{}+before:{}&hl={}'.format(
            key, start, end,  self.__lang.lower())
            ...
            for article in articles:
                try:
                    ...
                    try:
                        ...
                        datetime_obj = parse(
                            datetime_chars).replace(tzinfo=None)
                    except:
                        datetime_obj = None
                    # link
                    if deamplify:
                        try:
                            ...
                            print(deamp_e)
                            link = article.find("article").get(
                                "jslog").split('2:')[1].split(';')[0]
                    else:
                        link = 'news.google.com/' + \
                            article.find("h3").find("a").get("href")
